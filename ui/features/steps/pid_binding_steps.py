...
def step_impl(context):
    ...
    base_resolve_url = getattr(context.am_user.browser, 'base_resolve_url', nodice)
    pid_xml_namespace = getattr(context.am_user.browser, 'pid_xml_namespace', nodice)
    context.am_user.browser.configure_handle(**{
        # More runtime-specific parameters:
        'pid_web_service_endpoint': getattr(
            context.am_user.browser, 'pid_web_service_endpoint', nodice),
        'pid_web_service_key': getattr(
            context.am_user.browser, 'pid_web_service_key', nodice),
        'handle_resolver_url': getattr(
            context.am_user.browser, 'handle_resolver_url', nodice),
        'naming_authority': getattr(
            context.am_user.browser, 'naming_authority', '12345'),
        # Baked in:
