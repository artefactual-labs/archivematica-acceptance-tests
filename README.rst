...
``-D browser_name=Firefox``. The former ``driver_name`` option remains an alias
for Behave and direct Python callers; ``browser_name`` takes precedence when
both are supplied. To use a Chrome executable installed elsewhere,
...
After installing browsers, run ``make browser-test`` to check supported UI
layouts in Chrome and Firefox without a running Archivematica deployment.
