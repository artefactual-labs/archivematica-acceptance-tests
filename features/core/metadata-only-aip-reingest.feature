...
Feature: Metadata-only AIP re-ingest
  ...
  Scenario: Isla creates an AIP, and then performs a metadata-only re-ingest on it, adds metadata to it, and confirms that her newly added metadata are in the modified METS file.
    ...
    And the user waits for the "Perform policy checks on preservation derivatives?" decision point to appear and chooses "No" during transfer
    And the user waits for the "Perform policy checks on access derivatives?" decision point to appear and chooses "No" during transfer
