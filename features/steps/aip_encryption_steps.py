...
@given(
    "there is a standard GPG-encrypted AIP Storage location in the storage" " service"
)
def step_impl(context):
    context.execute_steps(
...
def step_impl(context, attributes):
    ...
    context.scenario.location_uuid = context.am_user.browser.ensure_ss_location_exists(
...
def step_impl(context, key_fname):
    ...
    context.scenario.import_gpg_key_result = context.am_user.browser.import_gpg_key(
        key_path
    )
...
@then(
    "the {aip_description} pointer file contains a PREMIS:OBJECT with a"
...
def step_impl(context, aips_store_path):
    ...
    if getattr(context.am_user.docker, "docker_compose_path", None):
        ...
        aip_local_path = context.am_user.ssh.scp_server_file_to_local(aip_server_path)
    else:
        aip_local_path = context.am_user.localfs.read_server_file(aip_server_path)
...
def assert_pointer_transform_file_encryption(pointer_path, ns, fingerprint=None):
