...
@given(
    "an Archivematica instance that {capture_output} client script output"
...
@when(
    "the user alters the Archivematica instance to {capture_output} client"
...
def step_impl(context, filename):
    ...
    data = {
        "mets": etree.tostring(
            utils.get_mets_from_scenario(context), pretty_print=True
        ).decode("utf8"),
...
@then(
    "the size of the {without_outputs_fname} METS file is less than that of"
...
def step_impl(context, without_outputs_fname, with_outputs_fname):
    ...
    logger.info("METS length for without output tasks: %d", without_outputs_mets_len)
    logger.info("METS length for with output tasks: %d", with_outputs_mets_len)
    assert without_outputs_mets_len < with_outputs_mets_len


@then(
    "the runtime of client scripts in {without_outputs_fname} is less"
