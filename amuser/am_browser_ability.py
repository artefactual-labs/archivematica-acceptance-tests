    ...
    def _submit_archival_storage_vue_search(self, aip_uuid):
        ...
        if not response.ok:
            raise ArchivematicaBrowserAbilityError(
                f"Archival Storage search failed with HTTP {response.status}"
            )
        # Read and validate the response before trusting the rendered results.
        response.json()
        live_status = self.page.locator("p.sr-only[aria-live='polite']")
        expect(live_status).to_have_text(
            re.compile(
                r"^(?:No results found|Showing [\d,]+ to [\d,]+ of [\d,]+ entries)$"
            ),
            timeout=self._milliseconds(self.pessimistic_wait),
        )

    def _search_archival_storage_vue(self, aip_uuid: str) -> bool:
        """Return whether a successful Vue search visibly contains this AIP."""
        query_inputs = self.page.locator("#search_form .aip-search-query-input")
        query_inputs.first.wait_for(state="visible")
        ...
        return bool(
            self.page.locator(
                f'a[href$="/archival-storage/{aip_uuid}/"]:visible'
            ).count()
        )

    def wait_for_aip_in_archival_storage(self, aip_uuid: str) -> None:
        """Wait for a visible AIP search result, failing if it never appears."""
        max_attempts = int(self.max_search_aip_archival_storage_attempts)
        for attempt in range(max_attempts):
            self.navigate(self.get_archival_storage_url(), reload=True)
            if self._search_archival_storage_vue(aip_uuid):
                return
            if attempt + 1 < max_attempts:
                self.wait(self.optimistic_wait)
        raise ArchivematicaBrowserAbilityError(
            f"AIP {aip_uuid} did not appear in Archival Storage "
            f"after {max_attempts} searches"
        )
