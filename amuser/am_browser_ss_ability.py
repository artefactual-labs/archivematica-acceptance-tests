...
SS_TABLE_LOAD_ERROR = "Unable to load records"
...
class ArchivematicaBrowserStorageServiceAbility(
    ...
    def _search_ss_table(self, search_term):
        ...
        if vue_table.count():
            self.wait_for_table_filter(
                ...
                search_term,
                ".ss-table-cell--empty",
                empty_error_text=SS_TABLE_LOAD_ERROR,
            )
            return
    ...
    def _get_ss_table_data_rows(self):
        ...
        for row in table.locator("tbody tr").all():
            ...
            if cells.count() == 1 and row_text == SS_TABLE_LOAD_ERROR:
                raise ArchivematicaBrowserStorageServiceAbilityError(row_text)
