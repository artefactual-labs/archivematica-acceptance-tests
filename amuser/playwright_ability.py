...
class ArchivematicaPlaywrightAbility(base.Base):
    ...
    def wait_for_table_filter(
        ...
        timeout=None,
        *,
        empty_error_text=None,
    ):
        """Wait until a client-side table reflects ``search_term``."""
        # FPR and Storage Service search inputs debounce updates by 100 ms
        # without exposing when the current query has been applied. Preserve
        # the minimum pre-migration settling interval before inspecting results.
        # The previous query's empty state (or matching rows) is not a ready signal.
        self.wait(0.3)
        tokens = [
        ...
        self.page.wait_for_function(
            r"""
            ({ tableSelector, emptySelector, emptyErrorText, tokens }) => {
              const empty = emptySelector
                ? document.querySelector(emptySelector)
                : null;
              if (empty) {
                const text = (empty.textContent || '').trim();
                if (emptyErrorText && text === emptyErrorText) {
                  throw new Error(text);
                }
                return /^no (?:matching records|records|results) found\.?$/i.test(text);
              }
