...
class ArchivematicaBrowserIngestAbility(
    ...
    def get_mets(self, transfer_name, sip_uuid=None, parse_xml=True):
        ...
        ms_name = utils.normalize_ms_name("Store AIP (review)")
        ...
        popup = None
        try:
            # Listen on the context because the popup's first response can
            # arrive before expect_popup returns. Read that response, not the
            # HTML that Chromium's XML viewer inserts into the document.
            with self.browser_context.expect_event(
                "response",
                predicate=lambda response: (
                    response.request.is_navigation_request()
                    and urlsplit(response.url).path.endswith("/filesystem/download_fs/")
            ...
            if response.frame.page != popup or not response.ok:
                raise ArchivematicaBrowserMETSAbilityError(
                    f"Unable to load the METS file from its popup (HTTP {response.status})."
                )
            mets = response.text()
        finally:
    ...
    def navigate_to_aip_directory_and_click(self, path):
        ...
        self._select_file_browser_path(self.page.locator("#aip-browser"), path)
    ...
    def parse_normalization_report(self, sip_uuid):
        ...
        self.navigate(self.get_ingest_url(), reload=True)
        ms_name = utils.normalize_ms_name("Approve normalization (review)")
